...
其余文件首先只发送相对路径、类型、大小和最多 2,000 字符的相关目录索引。只有模型明确要求补充内容时，才读取支持的文本文件前 1,200 字符；仍无法确定的文件标记 `REVIEW`，保留原位。PDF、Office、图片当前不提取正文或图像内容。
...
已有配置无需修改，新参数缺省即生效。`ai.max-requests=0` 可禁止本次 AI 请求，未被本地规则处理的文件会留待确认。字符预算不等于 token 或费用限额。文件组规划和结果缓存尚待下一轮实现。
